...
int64_t tickFct_CALCFSM(uint8_t btn) {
    static int32_t op, newOp, wait4num, newNum;
    static int64_t num,currentNum;
   ...
   switch(CALC_State) {
      ...
      case CALC_Num1:
         ...
         else if(currentNum>=0x10000000){ // too big
      ...
      case CALC_Op:
            if(newOp==13 && currentNum==0){
              ...
              wait4num=1; //operator has been pressed so we need a number next
              NEXT_State = CALC_Num2;
            }
         break;
      ...
      case CALC_Num2:
        ...
        else if(btn>= 10 && btn<=13 && wait4num==0){ // operator is pressed
           wait4num=1;
           NEXT_State = CALC_Op;
        }
        else if(currentNum>=0x10000000){ // Number is too big
           NEXT_State = CALC_TooBig;
        }
        else if(btn==15 && wait4num==0){ //equal is pressed
           NEXT_State = CALC_Solving;
        }
        else if(btn==13 && currentNum==0){// attempt to div by 0
            NEXT_State = CALC_Div0;
          }
        else if(btn<10){//number pressed
            wait4num=0;
            NEXT_State = CALC_Num2;
        }
        else{
      ...
      case CALC_Solved:
        if(btn==14){ // C
         ...
         else if(currentNum>=0x1000000){ // Number is too big
           NEXT_State = CALC_TooBig;
        }
        /*
        else if(btn>=10 && btn<=13){ // operator was pressed
   ...
   switch(CALC_State) {
      ...
      case CALC_Num1:
        if(btn<10){ // Another number is pressed
          if(!(currentNum>=0x7FFFFFFF && currentNum*10+ btn<=0x7FFFFFFF )){ // number is not too big
