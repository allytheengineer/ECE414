...
int64_t tickFct_CALCULATOR(uint8_t btn) {
   ...
   switch(CALC_State) {
      ...
      case CALC_Num1:
         if(btn>=A && btn<=D) // operator pressed?
      ...
      case CALC_Num2:
        ...
        else if(btn==14) // clr and move back to init
           CALC_State = CALC_Init;
   ...
   switch(CALC_State) {
      ...
      case CALC_Solving:
        if(op==A) // add
          ...
          currentNum = val * currentNum;
        else if(op==D && currentNum!=0) // divide not by 0
          currentNum = val / currentNum;
